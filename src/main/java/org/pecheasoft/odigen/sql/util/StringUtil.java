...
public class StringUtil {
    ...
    /**
     *
     * Given the time in long milliseconds, returns a String in the format X
     * hrs, Y mins, S sec, M msecs
     *
    ...
    /**
     * The following lines of code that deals with escape characters is mostly
     * copied from HIVE's FileUtils.java
     */
    ...
    public static String stringToHex(String str) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < str.getBytes().length; ) {
            byte chr = str.getBytes()[i];
            if (chr == 92) {
                byte nextChr = str.getBytes()[i+1];
                switch (nextChr) {
                    case 110 -> {
                        sb.append("\\u000A");
                        i=i+2;
                    ...
                    case 117 -> {
                        sb.append((char) chr);
                        sb.append((char) nextChr);
                        sb.append((char) str.getBytes()[i+2]);
                        sb.append((char) str.getBytes()[i+3]);
                        sb.append((char) str.getBytes()[i+4]);
                        sb.append((char) str.getBytes()[i+5]);
                        i=i+6;
                    }
                    default -> {
                        return str;
                ...
                sb.append(String.format("\\u%04x", chr));
                i++;
            }
        }
        return sb.toString();
     ...
     * @param defaultPath The default name for the path, if the given path is
     * empty or null.
    ...
    public static String unescapePathName(String path) {
        ...
        for (int i = 0; i < path.length(); i++) {
            ...
            if (c == '%' && i + 2 < path.length()) {
                int code;
                try {
                    code = Integer.parseInt(path.substring(i + 1, i + 3));
